/*
* drivers/cpufreq/cpufreq_smoothzen.c
*
...
static DEFINE_PER_CPU(struct smoothzen_info_s, smoothzen_info);
...
static int cpufreq_governor_smoothzen(struct cpufreq_policy *policy,
unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_SMOOTHZEN
static
#endif
struct cpufreq_governor cpufreq_gov_smoothzen = {
.name = "SmoothZen",
.governor = cpufreq_governor_smoothzen,
.max_transition_latency = 9000000,
.owner = THIS_MODULE,
};

static void cpufreq_smoothzen_timer(unsigned long data)
{
u64 delta_idle;
u64 update_time;
u64 now_idle;
struct smoothzen_info_s *this_smoothzen = &per_cpu(smoothzen_info, data);
struct cpufreq_policy *policy = this_smoothzen->cur_policy;

now_idle = get_cpu_idle_time_us(data, &update_time);

if (update_time == this_smoothzen->idle_exit_time)
return;

delta_idle = cputime64_sub(now_idle, this_smoothzen->time_in_idle);
//printk(KERN_INFO "smoothzen: t=%llu i=%llu\n",cputime64_sub(update_time,this_smoothzen->idle_exit_time),delta_idle);
...
this_smoothzen->force_ramp_up = 1;
...
*/
if (!timer_pending(&this_smoothzen->timer) && nr_running() > 0) {
this_smoothzen->time_in_idle = get_cpu_idle_time_us(
data, &this_smoothzen->idle_exit_time);
mod_timer(&this_smoothzen->timer, jiffies + sample_rate_jiffies);
}
...
{
struct smoothzen_info_s *this_smoothzen = &per_cpu(smoothzen_info, smp_processor_id());
struct cpufreq_policy *policy = this_smoothzen->cur_policy;
...
if (timer_pending(&this_smoothzen->timer) == 0) {
this_smoothzen->time_in_idle = get_cpu_idle_time_us(
smp_processor_id(), &this_smoothzen->idle_exit_time);
mod_timer(&this_smoothzen->timer, jiffies + sample_rate_jiffies);
}
}
...
*/
static unsigned int cpufreq_smoothzen_calc_freq(unsigned int cpu, struct cpufreq_policy *policy)
{
...
//printk(KERN_INFO "SmoothZen calc_freq: delta_time=%u cpu_load=%u\n",delta_time,cpu_load);
...
//printk(KERN_INFO "SmoothZen calc_freq: %u => %u\n",policy->cur,new_freq);
return new_freq;
...
static void cpufreq_smoothzen_freq_change_time_work(struct work_struct *work)
{
unsigned int cpu;
unsigned int new_freq;
struct smoothzen_info_s *this_smoothzen;
struct cpufreq_policy *policy;
cpumask_t tmp_mask = work_cpumask;
for_each_cpu(cpu, tmp_mask) {
this_smoothzen = &per_cpu(smoothzen_info, cpu);
policy = this_smoothzen->cur_policy;

if (this_smoothzen->force_ramp_up) {
this_smoothzen->force_ramp_up = 0;
...
new_freq = cpufreq_smoothzen_calc_freq(cpu,policy);
...
static struct attribute * smoothzen_attributes[] = {
...
static struct attribute_group smoothzen_attr_group = {
.attrs = smoothzen_attributes,
.name = "smoothzen",
};

static int cpufreq_governor_smoothzen(struct cpufreq_policy *new_policy,
unsigned int event)
{
unsigned int cpu = new_policy->cpu;
int rc;
struct smoothzen_info_s *this_smoothzen = &per_cpu(smoothzen_info, cpu);
...
if (this_smoothzen->enable) /* Already enabled */
break;
...
rc = sysfs_create_group(&new_policy->kobj, &smoothzen_attr_group);
if (rc)
return rc;
pm_idle_old = pm_idle;
pm_idle = cpufreq_idle;

this_smoothzen->cur_policy = new_policy;
this_smoothzen->enable = 1;

// notice no break here!

case CPUFREQ_GOV_LIMITS:
if (this_smoothzen->cur_policy->cur != new_policy->max)
__cpufreq_driver_target(new_policy, new_policy->max, CPUFREQ_RELATION_H);

break;

case CPUFREQ_GOV_STOP:
this_smoothzen->enable = 0;

if (atomic_dec_return(&active_count) > 1)
return 0;
sysfs_remove_group(&new_policy->kobj,
&smoothzen_attr_group);

pm_idle = pm_idle_old;
del_timer(&this_smoothzen->timer);
break;
}
...
static void smoothzen_suspend(int cpu, int suspend)
{
struct smoothzen_info_s *this_smoothzen = &per_cpu(smoothzen_info, smp_processor_id());
struct cpufreq_policy *policy = this_smoothzen->cur_policy;
unsigned int new_freq;

if (!this_smoothzen->enable || sleep_max_freq==0) // disable behavior for sleep_max_freq==0
return;
...
static void smoothzen_early_suspend(struct early_suspend *handler) {
int i;
suspended = 1;
for_each_online_cpu(i)
smoothzen_suspend(i,1);
}

static void smoothzen_late_resume(struct early_suspend *handler) {
int i;
suspended = 0;
for_each_online_cpu(i)
smoothzen_suspend(i,0);
}

static struct early_suspend smoothzen_power_suspend = {
.suspend = smoothzen_early_suspend,
.resume = smoothzen_late_resume,
};

static int __init cpufreq_smoothzen_init(void)
{
unsigned int i;
struct smoothzen_info_s *this_smoothzen;
...
this_smoothzen = &per_cpu(smoothzen_info, i);
this_smoothzen->enable = 0;
this_smoothzen->force_ramp_up = 0;
this_smoothzen->time_in_idle = 0;
this_smoothzen->idle_exit_time = 0;
// intialize timer:
init_timer_deferrable(&this_smoothzen->timer);
this_smoothzen->timer.function = cpufreq_smoothzen_timer;
this_smoothzen->timer.data = i;
}

/* Scale up is high priority */
up_wq = create_singlethread_workqueue("ksmoothzen_up");
down_wq = create_workqueue("ksmoothzen_down");

INIT_WORK(&freq_scale_work, cpufreq_smoothzen_freq_change_time_work);

register_early_suspend(&smoothzen_power_suspend);

return cpufreq_register_governor(&cpufreq_gov_smoothzen);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SMOOTHZEN
pure_initcall(cpufreq_smoothzen_init);
#else
module_init(cpufreq_smoothzen_init);
#endif

static void __exit cpufreq_smoothzen_exit(void)
{
cpufreq_unregister_governor(&cpufreq_gov_smoothzen);
destroy_workqueue(up_wq);
destroy_workqueue(down_wq);
}

module_exit(cpufreq_smoothzen_exit);

MODULE_AUTHOR ("Erasmux, modified by LeeDrOiD");
MODULE_DESCRIPTION ("'cpufreq_smoothzen' - A smart cpufreq governor");
MODULE_LICENSE ("GPL");
