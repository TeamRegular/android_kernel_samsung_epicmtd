/*
 * drivers/cpufreq/cpufreq_smartzen.c
 *
 ...
 *
 * For a general overview of smartzen see the relavent part in
...
static DEFINE_PER_CPU(struct smartzen_info_s, smartzen_info);
...
enum {
	SMARTZEN_DEBUG_JUMPS=1,
	SMARTZEN_DEBUG_LOAD=2,
	SMARTZEN_DEBUG_ALG=4
};
...
static int cpufreq_governor_smartzen(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_SMARTZEN
static
#endif
struct cpufreq_governor cpufreq_gov_smartzen = {
	.name = "SmartZen",
	.governor = cpufreq_governor_smartzen,
	.max_transition_latency = 9000000,
	.owner = THIS_MODULE,
};

inline static void smartzen_update_min_max(struct smartzen_info_s *this_smartzen, struct cpufreq_policy *policy, int suspend) {
	if (suspend) {
		this_smartzen->ideal_speed = // sleep_ideal_freq; but make sure it obeys the policy min/max
			policy->max > sleep_ideal_freq ?
			(sleep_ideal_freq > policy->min ? sleep_ideal_freq : policy->min) : policy->max;
	} else {
		this_smartzen->ideal_speed = // awake_ideal_freq; but make sure it obeys the policy min/max
			policy->min < awake_ideal_freq ?
			(awake_ideal_freq < policy->max ? awake_ideal_freq : policy->max) : policy->min;
	}
}

inline static void smartzen_update_min_max_allcpus(void) {
	unsigned int i;
	for_each_online_cpu(i) {
		struct smartzen_info_s *this_smartzen = &per_cpu(smartzen_info, i);
		if (this_smartzen->enable)
			smartzen_update_min_max(this_smartzen,this_smartzen->cur_policy,suspended);
	}
}
...
inline static void reset_timer(unsigned long cpu, struct smartzen_info_s *this_smartzen) {
	this_smartzen->time_in_idle = get_cpu_idle_time_us(cpu, &this_smartzen->idle_exit_time);
	mod_timer(&this_smartzen->timer, jiffies + sample_rate_jiffies);
}
...
inline static int target_freq(struct cpufreq_policy *policy, struct smartzen_info_s *this_smartzen,
			      int new_freq, int old_freq, int prefered_relation) {
	int index, target;
	struct cpufreq_frequency_table *table = this_smartzen->freq_table;
	    ...
	    !cpufreq_frequency_table_target(policy,table,new_freq,prefered_relation,&index))
	{
		...
		if (target == old_freq) {
			...
			printk(KERN_WARNING "SmartZen: frequency change failed: %d to %d => %d\n",
	...
	dprintk(SMARTZEN_DEBUG_JUMPS,"SmartZenQ: jumping from %d to %d => %d (%d)\n",
		old_freq,new_freq,target,policy->cur);

	return target;
}

static void cpufreq_smartzen_timer(unsigned long cpu)
{
	u64 delta_idle;
	u64 delta_time;
	int cpu_load;
	...
	u64 now_idle;
	int queued_work = 0;
	struct smartzen_info_s *this_smartzen = &per_cpu(smartzen_info, cpu);
	struct cpufreq_policy *policy = this_smartzen->cur_policy;

	now_idle = get_cpu_idle_time_us(cpu, &update_time);
	old_freq = policy->cur;

	if (this_smartzen->idle_exit_time == 0 || update_time == this_smartzen->idle_exit_time)
		return;

	delta_idle = cputime64_sub(now_idle, this_smartzen->time_in_idle);
	delta_time = cputime64_sub(update_time, this_smartzen->idle_exit_time);

	// If timer ran less than 1ms after short-term sample started, retry.
	if (delta_time < 1000) {
		if (!timer_pending(&this_smartzen->timer))
			reset_timer(cpu,this_smartzen);
		return;
	}
	...
	dprintk(SMARTZEN_DEBUG_LOAD,"smartzenT @ %d: load %d (delta_time %llu)\n",
		old_freq,cpu_load,delta_time);

	this_smartzen->cur_cpu_load = cpu_load;
	this_smartzen->old_freq = old_freq;
	...
	if (cpu_load > max_cpu_load || delta_idle == 0)
	{
		if (old_freq < policy->max &&
			 (old_freq < this_smartzen->ideal_speed || delta_idle == 0 ||
			  cputime64_sub(update_time, this_smartzen->freq_change_time) >= up_rate_us))
		{
			dprintk(SMARTZEN_DEBUG_ALG,"smartzenT @ %d ramp up: load %d (delta_idle %llu)\n",
				old_freq,cpu_load,delta_idle);
			this_smartzen->ramp_dir = 1;
			work_cpumask_set(cpu);
			queue_work(up_wq, &freq_scale_work);
			queued_work = 1;
		}
		else this_smartzen->ramp_dir = 0;
	}
	...
	// frequency we require that we have been at this frequency for at least down_rate_us:
		 ...
		 (old_freq > this_smartzen->ideal_speed ||
		  cputime64_sub(update_time, this_smartzen->freq_change_time) >= down_rate_us))
	{
		dprintk(SMARTZEN_DEBUG_ALG,"smartzenT @ %d ramp down: load %d (delta_idle %llu)\n",
			old_freq,cpu_load,delta_idle);
		this_smartzen->ramp_dir = -1;
		work_cpumask_set(cpu);
...
{
	struct smartzen_info_s *this_smartzen = &per_cpu(smartzen_info, smp_processor_id());
	struct cpufreq_policy *policy = this_smartzen->cur_policy;
	...
	if (policy->cur == policy->min && timer_pending(&this_smartzen->timer))
		del_timer(&this_smartzen->timer);

	pm_idle_old();

	if (!timer_pending(&this_smartzen->timer))
		reset_timer(smp_processor_id(), this_smartzen);
}

/* We use the same work function to sale up and down */
static void cpufreq_smartzen_freq_change_time_work(struct work_struct *work)
{
	unsigned int cpu;
	int new_freq;
	int old_freq;
	int ramp_dir;
	struct smartzen_info_s *this_smartzen;
	struct cpufreq_policy *policy;
	unsigned int relation = CPUFREQ_RELATION_L;
	for_each_possible_cpu(cpu) {
		this_smartzen = &per_cpu(smartzen_info, cpu);
		if (!work_cpumask_test_and_clear(cpu))
			continue;

		ramp_dir = this_smartzen->ramp_dir;
		this_smartzen->ramp_dir = 0;

		old_freq = this_smartzen->old_freq;
		policy = this_smartzen->cur_policy;

		if (old_freq != policy->cur) {
			// frequency was changed by someone else?
			printk(KERN_WARNING "SmartZen: frequency changed by 3rd party: %d to %d\n",
			       old_freq,policy->cur);
			new_freq = old_freq;
		}
		else if (ramp_dir > 0 && nr_running() > 1) {
			// ramp up logic:
			if (old_freq < this_smartzen->ideal_speed)
				new_freq = this_smartzen->ideal_speed;
			...
			dprintk(SMARTZEN_DEBUG_ALG,"smartzenQ @ %d ramp up: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_smartzen->ideal_speed);
		}
		else if (ramp_dir < 0) {
			// ramp down logic:
			if (old_freq > this_smartzen->ideal_speed) {
				...
				new_freq = old_freq * this_smartzen->cur_cpu_load / max_cpu_load;
				if (new_freq > old_freq) // min_cpu_load > max_cpu_load ?!
					new_freq = old_freq -1;
			}
			dprintk(SMARTZEN_DEBUG_ALG,"smartzenQ @ %d ramp down: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_smartzen->ideal_speed);
		}
			...
			new_freq = old_freq;
			dprintk(SMARTZEN_DEBUG_ALG,"smartzenQ @ %d nothing: ramp_dir=%d nr_running=%lu\n",
				old_freq,ramp_dir,nr_running());
		}

		// do actual ramp up (returns 0, if frequency change failed):
		new_freq = target_freq(policy,this_smartzen,new_freq,old_freq,relation);
		if (new_freq)
			this_smartzen->freq_change_time_in_idle =
				get_cpu_idle_time_us(cpu,&this_smartzen->freq_change_time);
		...
		else if (timer_pending(&this_smartzen->timer))
			del_timer(&this_smartzen->timer);
	}
}
...
static ssize_t store_sleep_ideal_freq(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	...
	if (res >= 0 && input >= 0) {
		...
		if (suspended)
			smartzen_update_min_max_allcpus();
	}
	return count;
}
...
static struct attribute * smartzen_attributes[] = {
...
static struct attribute_group smartzen_attr_group = {
	.attrs = smartzen_attributes,
	.name = "smartzen",
};

static int cpufreq_governor_smartzen(struct cpufreq_policy *new_policy,
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_smartzen->cur_policy = new_policy;

		this_smartzen->enable = 1;

		smartzen_update_min_max(this_smartzen,new_policy,suspended);

		this_smartzen->freq_table = cpufreq_frequency_get_table(cpu);
		if (!this_smartzen->freq_table)
			printk(KERN_WARNING "SmartZen: no frequency table for cpu %d?!\n",cpu);
		...
		if (this_smartzen->cur_policy->cur < new_policy->max && !timer_pending(&this_smartzen->timer))
			reset_timer(cpu,this_smartzen);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (this_smartzen->cur_policy->cur > new_policy->max) {
			dprintk(SMARTZEN_DEBUG_JUMPS,"SmartZenI: jumping to new max freq: %d\n",new_policy->max);
			__cpufreq_driver_target(this_smartzen->cur_policy,
						new_policy->max, CPUFREQ_RELATION_H);
		}
		else if (this_smartzen->cur_policy->cur < new_policy->min) {
			dprintk(SMARTZEN_DEBUG_JUMPS,"SmartZenI: jumping to new min freq: %d\n",new_policy->min);
			__cpufreq_driver_target(this_smartzen->cur_policy,
	...
	case CPUFREQ_GOV_STOP:
		this_smartzen->enable = 0;
		smp_wmb();
		del_timer(&this_smartzen->timer);
		flush_work(&freq_scale_work);
		this_smartzen->idle_exit_time = 0;
...
static void smartzen_suspend(int cpu, int suspend)
{
	struct smartzen_info_s *this_smartzen = &per_cpu(smartzen_info, smp_processor_id());
	struct cpufreq_policy *policy = this_smartzen->cur_policy;
	...
	smartzen_update_min_max(this_smartzen,policy,suspend);
	if (!suspend) { // resume at max speed:
		new_freq = validate_freq(policy,sleep_wakeup_freq);

		dprintk(SMARTZEN_DEBUG_JUMPS,"SmartZenS: awaking at %d\n",new_freq);
		...
		dprintk(SMARTZEN_DEBUG_JUMPS,"SmartZenS: suspending at %d\n",policy->cur);
	}

	reset_timer(smp_processor_id(),this_smartzen);
}

static void smartzen_early_suspend(struct early_suspend *handler) {
	int i;
	if (suspended || sleep_ideal_freq==0) // disable behavior for sleep_ideal_freq==0
		return;
	suspended = 1;
	for_each_online_cpu(i)
		smartzen_suspend(i,1);
}

static void smartzen_late_resume(struct early_suspend *handler) {
	int i;
	if (!suspended) // already not suspended so nothing to do
		return;
	suspended = 0;
	for_each_online_cpu(i)
		smartzen_suspend(i,0);
}

static struct early_suspend smartzen_power_suspend = {
	.suspend = smartzen_early_suspend,
	.resume = smartzen_late_resume,
#ifdef CONFIG_MACH_HERO
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1,
#endif
};

static int __init cpufreq_smartzen_init(void)
{
	unsigned int i;
	struct smartzen_info_s *this_smartzen;
	debug_mask = 0;
	...
	for_each_possible_cpu(i) {
		this_smartzen = &per_cpu(smartzen_info, i);
		this_smartzen->enable = 0;
		this_smartzen->cur_policy = 0;
		this_smartzen->ramp_dir = 0;
		this_smartzen->time_in_idle = 0;
		this_smartzen->idle_exit_time = 0;
		this_smartzen->freq_change_time = 0;
		this_smartzen->freq_change_time_in_idle = 0;
		this_smartzen->cur_cpu_load = 0;
		// intialize timer:
		init_timer_deferrable(&this_smartzen->timer);
		this_smartzen->timer.function = cpufreq_smartzen_timer;
		this_smartzen->timer.data = i;
		work_cpumask_test_and_clear(i);
	}

	// Scale up is high priority
	up_wq = create_singlethread_workqueue("ksmartzen_up");
	down_wq = create_workqueue("ksmartzen_down");
	if (!up_wq || !down_wq)
		return -ENOMEM;

	INIT_WORK(&freq_scale_work, cpufreq_smartzen_freq_change_time_work);

	register_early_suspend(&smartzen_power_suspend);

	return cpufreq_register_governor(&cpufreq_gov_smartzen);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SMARTZEN2
fs_initcall(cpufreq_smartzen_init);
#else
module_init(cpufreq_smartzen_init);
#endif

static void __exit cpufreq_smartzen_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_smartzen);
	destroy_workqueue(up_wq);
	destroy_workqueue(down_wq);
}

module_exit(cpufreq_smartzen_exit);

MODULE_AUTHOR ("Erasmux");
MODULE_DESCRIPTION ("'cpufreq_smartzen' - A smart cpufreq governor");
MODULE_LICENSE ("GPL");
